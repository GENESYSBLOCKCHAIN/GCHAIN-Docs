...
A snapshot is a recording of the state of Genesys Chain at a particular block height.
...
This snapshot can be used for jumpstarting a newly setup fullnode to avoid waiting a long time for getting all blocks. In Genesys Chain, the average block time is less than a second. As a result, it will take several weeks to sync from genesis block.
