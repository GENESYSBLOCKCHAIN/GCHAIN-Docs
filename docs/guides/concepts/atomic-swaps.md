...
```
AtomicSwapCoinsAccAddr = sdk.AccAddress(crypto.AddressHash([]byte("Genesys ChainAtomicSwapCoins")))
```
...
Hash Timer Locked Transfer (HTLT) is a new transaction type on Genesys Chain, to serve as HGSYS in the first step of Atomic Swap,
...
| ExpectedIncome | string | Expected income from swap counter party, example: "100:GSYS" or "100:GSYS,10000:BTCB-1DE" The amount needs to be bumped by e^8 | No |
...
Command line
```shell
./eth-cli token HTLT --recipient-addr <recipient-addr> --amount 100:GSYS --expected-income <expectedIncome> --height-span <heightSpan> --from <from-addr> --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```

Javascript
```javascript
  const client = new BncClient("https://testnet-dex.genesys.network")
  ...
  const expectedIncome = "100:GSYS"// expected income
...
2. Swap from Genesys Chain to Ethereum

* Clients send HTLT on Genesys Chain on *testnet*:

Command line:
```shell
./eth-cli token HTLT --from <from-addr> --chain-id GSYS-Chain-Ganges  --height-span <heightSpan> --amount <amount> --expected-income <expectedIncome> --recipient-addr <deputy-bep2-addr>  --recipient-other-chain <client ethereum address>  --cross-chain --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```
...
| Amount | Coins | The swapped out amount BEP2 tokens, example: "100:GSYS" or "100:GSYS,10000:BTCB-1DE" | No |
...
Command line:
```shell
./eth-cli token deposit --swap-id <swapID>  --amount 10000:TEST-599 --from <from-key> --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```
...
Command line:
```shell
./eth-cli token claim --swap-id  <swapID> --random-number <random-number> --from <from-key> --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```
...
Command line:
```shell
./eth-cli token refund --swap-id <swapID> --from <from-key>  --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```
...
* On testnet:
```
./eth-cli token query-swap --swap-id <swapID> --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```
Expected output
```json
{
...
* On testnet:
```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr <address> --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```
...
* On testnet:
```
./eth-cli token query-swapIDs-by-creator  --creator-addr <address> --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```
...
-- | -- | -- | --
HTLT | N/A |  0.000375 GSYS | Y
depositHTLT | N/A |  0.000375 GSYS | Y
claimHTLT | N/A |  0.000375 GSYS | Y
refundHTLT | N/A |  0.000375 GSYS | Y
...
* Its corresponding address on testnet is: `tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr`on Genesys Chain and `0xD93395B2771914E1679155F3EA58C41d89D96098` on Ethereum testnet

### Swap Tokens from Ethereum to Genesys Chain
...
#### 3. Deputy Call HTLT on Genesys Chain
Then, Deputy will send `HTLT` transaction [here](https://testnet-explorer.genesys.network/tx/99CBC2896F0CF14DDAB0684BDA0A3E9FF2271056E68EC3559AB7FB24E0EE97DE)

#### 4. Claim HTLT on Genesys Chain

* Get the `swapID` on Genesys Chain

```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr tbnb1cs0j4p0p6d3fvd77zg3qzlwwgmrv3e9e63423w --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
[
...
* Send claim transaction on Genesys Chain

```
./eth-cli token claim --swap-id  12aacc3bdc2cef97e8e45cc9b409796df57904a4e9c76863ad8420ff75f13128  --random-number <random-number> --from <from-key>  --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```

Example of `claim` tx on [testnet](https://testnet-dex.genesys.network/api/v1/tx/6BA714E6D107F1D9634DDC159F560A1FB61393B8E15723EFD70B9EA8B0B1AA9A?format=json)
...
This is a javascript implementation for client app to swap [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) to [PPC-00A](https://testnet-explorer.genesys.network/asset/PPC-00A) with deputy.
...
```javascript
  ...
  const deputyGSYSWalletAddr = "tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr"
  ...
  const bnbClient = new BncClient("https://testnet-dex.genesys.network")
  ...
  const bnbRPC = new rpcClient("https://seed-pre-s3.genesys.network", "testnet")
  ...
  const hexEncodingClientGSYSaddr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  ...
  const htltData = swapContract.methods.htlt("0x"+randomNumberHash, timestamp, heightSpan, deputyEthWalletAddr, "0x0", hexEncodingClientGSYSaddr, amount, expectedIncome).encodeABI()
  ...
  //----------------------------------------------------------------------------
  //Step3 query swap created by deputy on Genesys Chain and verify swap parameters
  //----------------------------------------------------------------------------
  const swapID = calculateSwapID(randomNumberHash.replace("0x", ""), deputyGSYSWalletAddr, clientEthWalletAddr).toString()
  console.log(swapID)
...
### Swap Tokens from Genesys Chain to Ethereum
![image-20190918193910521](../../assets/bnc2eth.png)

#### 1. Send `HTLT` Transaction from Genesys Chain
...
Example is [here](https://testnet-explorer.genesys.network/tx/9ECECE9E0F08EE78583CFA37FD4C3F03521289F0F229A612886B8B21B9C62D7F)

Then, you can query the the swap by `SwapID`:

```
./eth-cli token query-swap --swap-id f85dd907df0a5897927b949c0f9e2563d453ba698ff9941fed1ce91f8057afc2 --chain-id GSYS-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.genesys.network:80
```
...
- `_randomNumberHash` must equal to the randomNumberHash in client HTLT transaction on Genesys Chain
- `_recipientAddr` must equal to client ethereum wallet address
- `_timestamp` must equal to the timestamp in client HTLT transaction on Genesys Chain
...
#### 5. Deputy Claim on Genesys Chain

`Claim HTLT` transaction from **Deputy** is [sent](https://testnet-explorer.genesys.network/tx/8C616DEFD2EAA41E13D2DC4844B218DFF8CFE24B4C7A693AAD700381B5FF7B48) afterwards:

#### 6. Demo for Client APP: swap bep2 to erc20

This is a javascript implementation for client app to swap  [PPC-00A](https://testnet-explorer.genesys.network/asset/PPC-00A) to [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) with deputy.
...
```javascript
  ...
  //--------------------------------------------
  //Step1 send htlt on Genesys Chain
  //--------------------------------------------
  const randomNumber = "e8eae926261ab77d018202434791a335249b470246a7b02e28c3b2fb6ffad8f3"
  const timestamp = Math.floor(Date.now()/1000)
  const randomNumberHash = calculateRandomNumberHash(randomNumber, timestamp).toString("hex")
  ...
  bnbClient.swap.HTLT(clientBnbWalletAddr, deputyGSYSWalletAddr, clientEthWalletAddr, "", randomNumberHash, timestamp, amount, expectedIncome, heightSpan, true)
  await wait(1000)
  ...
  //----------------------------------------------------------------------------
  const hexEncodingClientGSYSaddr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  const swapID = await swapContract.methods.calSwapID("0x"+randomNumberHash, deputyEthWalletAddr, hexEncodingClientGSYSaddr).call()
  console.log(swapID)
  ...
  //----------------------------------------------------------------------------
  //If step2 or step3 are failed and the expire height on Genesys Chain is passed, try to send refundHTLT transaction on Genesys Chain
  //----------------------------------------------------------------------------
```
