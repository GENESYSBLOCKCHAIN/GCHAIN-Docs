...
BLUE721 is a standard for representing ownership of [non-fungible tokens](https://docs.openzeppelin.com/contracts/3.x/tokens#different-kinds-of-tokens), that is, where each token is unique such as in real estate or collectibles.

We will use [Presets](https://docs.openzeppelin.com/contracts/3.x/api/presets) contracts in [OpenZeppelin Contracts](https://docs.openzeppelin.com/contracts/3.x/) to create an BLUE721 and deploy using Truffle.
...
Then we install [OpenZeppelin Contracts](https://docs.openzeppelin.com/contracts) which has an implementation of BLUE721.
...
We are going to use Preset [`ERC721PresetMinterPauserAutoId`](https://docs.openzeppelin.com/contracts/3.x/api/presets#ERC721PresetMinterPauserAutoId) which is an BLUE721 that is preset so it can be minted (with auto token ID and metadata URI), paused and burned.
...
Next we will deploy to Genesys Chain  testnet .
...
```
    ...
    nc: {
      provider: () => new HDWalletProvider(mnemonic, `https://mainnet-rpc.genesys.network`),
      network_id: 56,
      confirmations: 10,
      timeoutBlocks: 200,
      skipDryRun: true
