...
GSYS and coins on Genesys Chain are supported by the Ledger Nano S hardware wallet. The following describes how to use Ledger Nano S to connect to TC
...
!!! Tip
		The existing Ethereum App is compatible with Genesys Chain
...
2) Within the "Manager" pane, type in "Ethereum" in the search field. Locate "Ethereum", then click on "Install". (Note: Do not use the "Genesys Chain" App)
...
## Enabling contract data for BLUE20 
...
Genesys Chain is compatible with MetaMask. Users can use MetaMask as a bridge between Ledger Nano S and Genesys Chain to make transactions to the Genesys Chain testnet.

1) Connect [MetaMask](./metamask.md) to GSYS Testnet by following this guide to add a custom network
...
1. Fail to tranfer BLUE20 tokens
